...
alter table taxonomy_node add _out_target_parent as rtrim(ltrim(reverse(substring(replace(reverse(out_target),';',replicate(' ',1000)),500,1500)))) PERSISTED

alter table taxonomy_node add _out_target_name as rtrim(ltrim(reverse(substring(replace(reverse(out_target),';',replicate(' ',1000)),0,500)))) PERSISTED

exec refresh_views

-- *******************************************************************************************************
-- update [build_delta_nodes] to use out_target_name
-- *******************************************************************************************************

GO

/****** Object:  StoredProcedure [dbo].[rebuild_delta_nodes]    Script Date: 4/16/2020 4:44:00 PM ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	insert into taxonomy_node_delta (prev_taxid, new_taxid, proposal, notes, is_new, is_split, is_now_type)
	select 
	...
	insert into taxonomy_node_delta (prev_taxid, new_taxid, proposal, notes, is_renamed, is_merged, is_promoted, is_moved, is_now_type, is_deleted)
