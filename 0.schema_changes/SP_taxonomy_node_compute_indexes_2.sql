...
GO
--
-- add inher_molecule_src
-- add child_count
--
-- requires schema changes to taxonomy_node (add 2 columns) to implement. 
--
/****** Object:  StoredProcedure [dbo].[taxonomy_node_compute_indexes]    Script Date: 10/17/2018 1:39:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE
PROCEDURE [dbo].[taxonomy_node_compute_indexes_2] 
	@taxnode_id int,
	@left_idx  int = 1,
...
AS	
	...
	*/
	...
	SELECT 
		...
		,@inher_molecule_src  = ISNULL((case when self.molecule_id is not null then self.taxnode_id end),@inher_molecule_src)
	...
	SET 
		...
		, inher_molecule_src=@inher_molecule_src
		, lineage = @my_lineage
	...
	or (inher_molecule_src <> @inher_molecule_src or (inher_molecule_src is null and @inher_molecule_src is not null))
	...
	-- set our RIGHT index
	UPDATE taxonomy_node
	SET right_idx = @right_idx, child_count=@child_count
	WHERE taxnode_id = @taxnode_id
