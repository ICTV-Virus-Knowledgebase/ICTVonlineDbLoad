USE [ICTVonlnie34]
GO
/****** Object:  StoredProcedure [dbo].[sp_simplify_molecule_id_settings]    Script Date: 4/24/2020 3:40:38 PM ******/
SET ANSI_NULLS ON
GO
...
WHILE(1=1) BEGIN -- run until 0 rows change
	...
	-- select 	taxonomy_node.level_id, taxonomy_node.lineage, 
	...
	join (
		...
		where t.tree_id = 20180000 --dbo.udf_getTreeID(NULL) 
		and t.level_id >= 200
...
WHILE(1=1) BEGIN
	...
	--
	update taxonomy_node set
	--select taxonomy_node.lineage, taxonomy_node.molecule_id, p.lineage, p.inher_molecule_id,
		molecule_id = NULL
	from taxonomy_node 
	join taxonomy_node p on p.taxnode_id = taxonomy_node.parent_id
	where taxonomy_node.molecule_id = p.inher_molecule_id
	and  taxonomy_node.tree_id=20180000 --dbo.udf_getTreeID(NULL)
	--print @@ROWCOUNT
