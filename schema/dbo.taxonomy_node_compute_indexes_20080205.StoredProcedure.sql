USE [ICTVonlnie34]
GO
/****** Object:  StoredProcedure [dbo].[taxonomy_node_compute_indexes_20080205]    Script Date: 4/24/2020 3:40:38 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[taxonomy_node_compute_indexes_20080205] 
	@taxnode_id int = NULL,
	@left_idx  int = 1,
...
AS	
	...
	IF @tree_id IS NULL
		SELECT @tree_id = self.taxnode_id
		FROM taxonomy_node self 
		JOIN taxonomy_level level on level.id = self.level_id
		WHERE	level.name =  'tree'
