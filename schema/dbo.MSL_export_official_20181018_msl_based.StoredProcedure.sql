...
CREATE procedure [dbo].[MSL_export_official_20181018_msl_based]
	@msl int = NULL
as
 -- -------------------------------------------------------------------------
...
select 
	...
	, 'New MSL including all taxa updates since the '+(select LEFT(rtrim(tree_id),4) from taxonomy_node where level_id=100 and msl_release_num=(@msl-1))+' release' as cell_6E
