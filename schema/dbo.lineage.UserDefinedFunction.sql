USE [ICTVonlnie34]
GO
/****** Object:  UserDefinedFunction [dbo].[lineage]    Script Date: 4/24/2020 3:40:38 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	FETCH NEXT FROM my_cursor INTO @node_name, @level_name, @is_hidden
	  ...
	  IF  @is_hidden = 0 BEGIN 
		SET @lineage = CASE
			WHEN @list_format = 'named' THEN
				ISNULL(@lineage,'') + @cur_sep + @level_name + '=' + ISNULL(@node_name,'NULL')
			ELSE
				ISNULL(@lineage,'') + @cur_sep + ISNULL(@node_name,'NULL')
			END
	   ...
	   INTO @node_name, @level_name, @is_hidden
	END
