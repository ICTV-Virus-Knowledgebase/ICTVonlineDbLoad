...
--
-- UPDATE - make "only child" non-type species, the type species for their genus.
--
--
select taxnode_id, lineage, is_ref
-- update species set is_ref=1
from taxonomy_node species
where species.taxnode_id in (
	select 
		top 10000 -- so can leave the order by here
		max(species.taxnode_id)
		--DEBUG-- select 'genera with only-child non-type species', genus.msl_release_num, genus.lineage, genus.name, species_ct = count(species.taxnode_id), type_species_ct=sum(species.is_ref), min(species.taxnode_id), max(species.taxnode_id)
	...
	group by genus.msl_release_num, genus.taxnode_id, genus.name, genus.lineage, genus.left_idx
	having 
		sum(species.is_ref)=0 and  count(species.taxnode_id)=1
	order by 
	--	genus.lineage,
		genus.msl_release_num desc,
		sum(species.is_ref) desc
)
