...
/****** Object:  UserDefinedFunction [dbo].[udf_getChildTaxaCounts]    Script Date: 4/21/2020 11:59:50 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE or ALTER FUNCTION [dbo].[udf_rankCountsToStringWithPurals]
(
        @realm_ct as int,
        @subrealm_ct as int,
        @kingdom_ct as int,
...
BEGIN
	/*
	 * this functions makes an English DISPLAY STRING (ick)
	 * that lists all the rank counts given with appropriate pluralization
	 *
	 * it is used by the SP taxonomy_node_compute_indexes to populate the columns
	 *		taxonomy_node.taxa_kid_cts & taxonomy_node.taxa_desc_cts
	 * which is called by the update trigger on taxonomy_node.
	 *
	 */
